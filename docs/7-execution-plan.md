...
**완료 조건:**
- [ ] PostgreSQL 17 설치 확인 (`psql --version` → 17.x)
- [ ] 애플리케이션 전용 사용자 생성: `todolist_user`
- [ ] 개발 DB 생성: `todolist_dev`
- [ ] 테스트 DB 생성: `todolist_test`
- [ ] `todolist_user`로 두 DB 접속 가능 확인
...
**완료 조건:**
- [ ] `backend/.env.example` 생성 (키 목록, 실제값 제외)
- [ ] `backend/.env` 생성 (개발 환경 실제값 포함)
  ```
...
- [ ] `frontend/.env.example` 생성 (`VITE_API_URL=http://localhost:3000`)
...
**완료 조건:**
- [ ] `psql -U todolist_user -d todolist_dev -f database/schema.sql` 성공 실행
- [ ] `psql -U todolist_user -d todolist_test -f database/schema.sql` 성공 실행
- [ ] 3개 테이블 생성 확인 (`\dt`: users, categories, todos)
- [ ] 인덱스 6개 생성 확인 (`\di`)
...
- [ ] 트리거 2개 생성 확인: `trg_users_updated_at`, `trg_todos_updated_at`
...
- [ ] 기본 카테고리 3개 삽입 확인
  ...
  ```
- [ ] `user_id = NULL`, `is_default = TRUE` 확인
- [ ] 외래키 ON DELETE RESTRICT 확인 (`\d todos`의 FK 설정)
...
**완료 조건:**
- [ ] `backend/src/config/env.ts` 작성
- [ ] 필수 환경변수 목록 검증: `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `JWT_SECRET`, `PORT`
- [ ] 누락 시 서버 시작 중단 및 명확한 에러 메시지 출력
...
**완료 조건:**
- [ ] `npm run db:test` 실행 성공
- [ ] `SELECT COUNT(*) FROM categories WHERE is_default = TRUE;` → 결과 3
- [ ] "Connected to PostgreSQL successfully" 메시지 출력
...
- [ ] 모든 쿼리 `$1, $2, ...` 파라미터화 (SQL Injection 방지)
- [ ] 반환 타입 `UserRow` 인터페이스 정의
...
- [ ] NULL 조건 처리: `WHERE user_id = $1 OR (user_id IS NULL AND is_default = TRUE)`
- [ ] 모든 쿼리 파라미터화 확인
...
- [ ] `backend/src/repositories/todo.repository.ts` 구현
  - `findByUserId(userId, filters?)` — 동적 필터 조회 (BR-F-01~F-04)
...
- [ ] 동적 필터 쿼리: 조건 배열 + 파라미터 인덱스 자동 증가 패턴
  ```typescript
  const conditions: string[] = ['user_id = $1'];
  const values: unknown[] = [userId];
  let idx = 2;
  if (filters?.categoryId) { conditions.push(`category_id = $${idx++}`); values.push(filters.categoryId); }
  ```
- [ ] 모든 쿼리 파라미터화 확인
...
**완료 조건:**
- [ ] `backend/package.json` 생성 (Node.js LTS)
- [ ] 필수 패키지 설치: `express`, `typescript`, `ts-node`, `pg`, `jsonwebtoken`, `bcrypt`, `dotenv`, `cors`
- [ ] 개발 의존성: `jest`, `supertest`, `@types/*`
- [ ] `tsconfig.json` — strict mode 활성화
- [ ] `npm scripts` 정의: `start`, `dev`, `build`, `test`
...
**완료 조건:**
- [ ] `src/app.ts` 작성 — CORS, `express.json()`, 라우팅 통합, 404 핸들러
- [ ] `src/index.ts` 작성 — 환경변수 검증 → DB 연결 확인 → 서버 시작
- [ ] `src/utils/logger.ts` 작성 — `error()`, `warn()`, `info()`, `debug()` 메서드 (`LOG_LEVEL` 제어)
- [ ] ESLint `no-console` 규칙 활성화 (logger 사용 강제)
...
**완료 조건:**
- [ ] `src/utils/jwt.utils.ts` 구현
  - `signToken(userId: string): string`
  - `verifyToken(token: string): { userId: string } | null`
  - `extractToken(authHeader: string): string | null`
- [ ] 테스트: 토큰 생성/검증 성공, 만료 토큰 실패, 위조 토큰 실패
...
**완료 조건:**
- [ ] `src/utils/password.utils.ts` 구현
  - `hashPassword(plain: string): Promise<string>` — bcrypt, cost factor ≥ 10
  - `comparePassword(plain: string, hashed: string): Promise<boolean>`
- [ ] 테스트: 해시 생성 및 검증 성공, 잘못된 비밀번호 실패
...
**완료 조건:**
- [ ] `src/middlewares/auth.middleware.ts` 구현
  - `Authorization: Bearer <token>` 헤더 검증
  - 검증 성공 시 `req.user = { userId }` 설정
  - 실패 시 401 Unauthorized 반환
- [ ] `src/types/express.d.ts` — `Request` 타입에 `user?: { userId: string }` 추가
- [ ] 테스트: 유효 토큰 통과, 토큰 없음/위조 401
...
**완료 조건:**
- [ ] `src/utils/app-error.ts` — `AppError(statusCode, message, code?)` 커스텀 클래스
...
**완료 조건:**
- [ ] `src/middlewares/validate.middleware.ts` 구현
...
**완료 조건:**
- [ ] `src/utils/date.utils.ts` 구현
  - `isValidFutureDate(dateString): boolean` — ISO 형식 + 오늘 이후 (BR-T-06)
  - `isOverdue(dueDate: Date | null): boolean` — `dueDate < CURRENT_DATE` (BR-F-02)
  - `getTodayDate(): Date` — 시간 제외 날짜 기준

---

### BE-10: TypeScript 타입 정의
**의존**: BE-02

**완료 조건:**
- [ ] `src/types/user.types.ts` — `UserRow`, `UserDto`, `CreateUserRequest`, `UpdateUserRequest`
- [ ] `src/types/todo.types.ts` — `TodoRow`, `TodoDto`, `CreateTodoRequest`, `UpdateTodoRequest`, `TodoFilters`
- [ ] `src/types/category.types.ts` — `CategoryRow`, `CategoryDto`, `CreateCategoryRequest`
- [ ] `src/types/api.types.ts` — 공통 Response/Error 타입
...
- [ ] Service: `registerUser()` — 이메일 중복 확인(BR-U-01), bcrypt 해싱(BR-U-02)
- [ ] 응답: 성공 201 `{ "message": "회원가입이 완료되었습니다" }`
- [ ] 실패: 이메일 중복 400, 입력 누락 400
- [ ] 통합 테스트 통과
...
- [ ] Service: `authenticateUser()` — 비밀번호 검증, JWT 발급
- [ ] 실패 시 단일 에러 메시지 (계정 존재 여부 노출 금지)
- [ ] 응답: 성공 200 `{ "accessToken": "..." }`
- [ ] 통합 테스트 통과
...
**완료 조건:**
- [ ] Route: `GET /api/users/me` (인증 필요)
- [ ] 응답: 200 `{ id, email, name, created_at, updated_at }` (password 제외)
- [ ] 미인증 401
...
**완료 조건:**
- [ ] Route: `PATCH /api/users/me` (인증 필요)
- [ ] 이메일 변경 요청 시 400 거부 (BR-U-04)
...
**완료 조건:**
- [ ] Route: `GET /api/categories` (인증 필요)
- [ ] 기본 카테고리(is_default=true) + 현재 사용자 정의 카테고리 통합 반환
- [ ] 응답: 200, 카테고리 배열 (`is_default` 필드 포함)
- [ ] 미인증 401
...
**완료 조건:**
- [ ] Route: `POST /api/categories` (인증 필요)
- [ ] 동일 사용자 내 이름 중복 시 400 (BR-C-04)
- [ ] 응답: 성공 201 `{ id, name, is_default, created_at }`
- [ ] 통합 테스트 통과
...
**완료 조건:**
- [ ] Route: `POST /api/todos` (인증 필요)
- [ ] `title`, `category_id` 필수 (BR-T-01, BR-T-02)
- [ ] `due_date` 입력 시 오늘 이후 검증 (BR-T-06)
- [ ] 카테고리 접근 권한 검증 (기본 카테고리 전체 공유, 사용자 정의는 소유자만 — BR-C-02)
- [ ] 응답: 성공 201, 전체 Todo 객체
- [ ] 통합 테스트 통과
...
**완료 조건:**
- [ ] Route: `GET /api/todos` (인증 필요)
- [ ] 쿼리 파라미터: `?category_id=&is_completed=&overdue=`
- [ ] 복수 필터 AND 조건 적용 (BR-F-04)
- [ ] 정렬: `created_at DESC`
- [ ] 미인증 401
...
**완료 조건:**
- [ ] Route: `PATCH /api/todos/:id` (인증 필요)
- [ ] 소유권 검증 — 타인 소유 시 403 (BR-T-03, 존재 여부 노출 금지)
- [ ] `due_date` 수정 시 오늘 이후 검증 (BR-T-06)
- [ ] 카테고리 변경 시 접근 권한 검증 (BR-C-02)
- [ ] 응답: 성공 200, 수정된 Todo 객체
...
**완료 조건:**
- [ ] Route: `PATCH /api/todos/:id/complete` (인증 필요)
- [ ] Request: `{ "is_completed": boolean }`
- [ ] 완료 처리: `completed_at = NOW()` 자동 기록 (BR-T-04)
- [ ] 미완료 복원: `completed_at = NULL` 초기화 (BR-T-05)
- [ ] 소유권 검증, 타인 소유 시 403 (BR-T-03)
- [ ] 통합 테스트 통과
...
**완료 조건:**
- [ ] Route: `DELETE /api/todos/:id` (인증 필요)
- [ ] 소유권 검증 — 타인 소유 시 403 (BR-T-03)
- [ ] 응답: 성공 200 `{ "message": "할일이 삭제되었습니다" }`
- [ ] 통합 테스트 통과
...
**완료 조건:**
- [ ] `src/routes/index.ts` — 모든 라우터 통합
- [ ] `/api/auth/*`, `/api/users/*`, `/api/categories/*`, `/api/todos/*` 경로 등록
- [ ] 인증 필요 라우터에 `authMiddleware` 적용
- [ ] 전역 에러 미들웨어 마지막 등록 확인
- [ ] `npm run dev` 실행 후 서버 정상 시작 확인
...
**완료 조건:**
- [ ] `tests/unit/services/` 아래 서비스별 테스트 파일 작성
  - `auth.service.test.ts` — 회원가입/로그인 성공·실패
  - `todo.service.test.ts` — CRUD, 소유권(BR-T-03), 완료처리(BR-T-04/05)
  - `category.service.test.ts` — 생성, 이름 중복(BR-C-04)
  - `user.service.test.ts` — 프로필 조회/수정
- [ ] `npm test` 실행 시 모든 단위 테스트 통과
...
**완료 조건:**
- [ ] `tests/integration/` 아래 API 엔드포인트 전체 통합 테스트 작성 (Supertest)
- [ ] 모든 인증 필요 엔드포인트 401 테스트
- [ ] 소유권 검증 403 테스트
- [ ] `npm test` 실행 시 모든 통합 테스트 통과
...
- [ ] 필수 패키지 설치: `react-router-dom`, `zustand`, `@tanstack/react-query`
- [ ] `tsconfig.json` — strict mode 활성화
- [ ] ESLint + Prettier 설정
...
- [ ] `src/stores/authStore.ts` 구현
  ...
  ```
- [ ] localStorage/sessionStorage/Cookie **미사용** 확인 (메모리만)
- [ ] `setAuth`, `clearAuth` 동작 테스트
...
- [ ] `src/constants/queryKeys.constants.ts` 생성 — QUERY_KEYS 중앙 관리
  ```typescript
  QUERY_KEYS = {
...
**완료 조건:**
- [ ] `src/types/api.types.ts` — 공통 Response/Error 타입
- [ ] `src/features/auth/auth.types.ts` — User, LoginRequest, RegisterRequest, AuthResponse
- [ ] `src/features/todo/todo.types.ts` — Todo, CreateTodoRequest, UpdateTodoRequest, TodoFilters
- [ ] `src/features/category/category.types.ts` — Category, CreateCategoryRequest
- [ ] `src/features/user/user.types.ts` — UserProfile, UpdateUserRequest
...
- [ ] `PrivateRoute` — accessToken 없으면 `/login` 리다이렉트
- [ ] `PublicRoute` — accessToken 있으면 `/` 리다이렉트
...
**완료 조건:**
- [ ] `src/api/auth.api.ts` — `register()`, `login()`
- [ ] `src/api/todo.api.ts` — `getTodos(filters?)`, `createTodo()`, `updateTodo()`, `deleteTodo()`, `toggleTodoComplete()`
- [ ] `src/api/category.api.ts` — `getCategories()`, `createCategory()`
- [ ] `src/api/user.api.ts` — `getMe()`, `updateProfile()`
- [ ] 모든 함수 TypeScript 타입 안전성 확보
...
**완료 조건:**
- [ ] `useRegister()` — mutation, 성공 시 authStore.setAuth()
- [ ] `useLogin()` — mutation, 성공 시 authStore.setAuth()
- [ ] 에러 시 에러 메시지 반환
...
**완료 조건:**
- [ ] `useTodos()` — filterStore 구독, 필터 변경 시 자동 재쿼리
- [ ] `useCreateTodo()` — 성공 시 `QUERY_KEYS.todos.all` 무효화
- [ ] `useUpdateTodo()` — 성공 시 캐시 무효화
- [ ] `useDeleteTodo()` — 성공 시 캐시 무효화
- [ ] `useToggleTodoComplete()` — 낙관적 업데이트 구현
...
**완료 조건:**
- [ ] `useCategories()` — staleTime 60초, 인증 상태 시 활성화
- [ ] `useCreateCategory()` — 성공 시 `QUERY_KEYS.categories.all` 무효화, 중복 이름 에러 핸들링 (BR-C-04)
...
**완료 조건:**
- [ ] `useUpdateProfile()` — 이름/비밀번호 수정 mutation, 성공 시 `QUERY_KEYS.user.me` 무효화
...
**완료 조건:**
- [ ] `Button.tsx` — variant (primary, secondary, danger), disabled, loading 상태
- [ ] `Input.tsx` — type, error 상태, label 연결
- [ ] `ErrorMessage.tsx` — 에러 메시지 표시 (빨강)
- [ ] `EmptyState.tsx` — 필터 결과 없을 때 표시
- [ ] 접근성: label-input 연결, aria-label 속성
...
- [ ] `Header.tsx` — 사용자명 표시, 로그아웃 버튼, 프로필 링크
- [ ] 로그아웃: `authStore.clearAuth()` + `/login` 리다이렉트
...
- [ ] `src/utils/error.utils.ts` — API 에러 코드 → 사용자 친화적 메시지 매핑
...
**완료 조건:**
- [ ] `LoginPage.tsx` + `LoginForm.tsx` 구현
- [ ] 폼 유효성 검증: email 형식, password 필수
- [ ] 제출: `useLogin()` 호출
- [ ] 성공: `/` 리다이렉트
- [ ] 실패: 단일 에러 메시지 (계정 존재 여부 노출 금지)
- [ ] 로딩 중 버튼 disabled
- [ ] 회원가입 링크 (`/register`)
...
**완료 조건:**
- [ ] `RegisterPage.tsx` + `RegisterForm.tsx` 구현
- [ ] 폼 유효성 검증: email 형식, password 8자↑, 비밀번호 확인 일치, name 필수
- [ ] 제출: `useRegister()` 호출
- [ ] 성공: `/login` 리다이렉트 + 안내 메시지
- [ ] 실패: 이메일 중복 오류 메시지
- [ ] 로그인 링크 (`/login`)
...
**완료 조건:**
- [ ] `CategoryList.tsx` 구현 — 기본/사용자 정의 카테고리 구분 표시
- [ ] 클릭 시 `filterStore.setFilter({ categoryId })` 호출
- [ ] "전체" 선택: `filterStore.setFilter({ categoryId: null })`
- [ ] 선택된 카테고리 하이라이트
- [ ] `AddCategoryForm.tsx` — 이름 입력, `useCreateCategory()` 호출, 중복 에러 메시지
...
- [ ] 각 필터 변경 시 `filterStore.setFilter()` 호출
- [ ] 필터 초기화 버튼: `filterStore.resetFilter()`
...
**완료 조건:**
- [ ] `TodoForm.tsx` — 생성/수정 모드 겸용 (id props 여부로 분기)
- [ ] 필수 입력: title, category_id
- [ ] 선택 입력: description, due_date
- [ ] `due_date`: 오늘 이후 검증 (BR-T-06), 과거 날짜 입력 시 에러 메시지
- [ ] `CreateTodoForm.tsx` — 버튼 + 모달/인라인 폼
- [ ] `EditTodoForm.tsx` — 기존 값 초기화 (prepopulate)
...
**완료 조건:**
- [ ] `TodoListPage.tsx` 구현 — Layout 적용
- [ ] `TodoList.tsx` — `useTodos()` 결과 렌더링, 로딩/에러/빈 상태 표시
- [ ] 필터 변경 시 자동 목록 갱신 확인
...
**완료 조건:**
- [ ] `ProfilePage.tsx` + `ProfileForm.tsx` 구현
- [ ] 이메일: 읽기 전용 표시 (BR-U-04)
- [ ] 이름 변경 폼: `useUpdateProfile()` 호출
- [ ] 비밀번호 변경 폼: 현재 비밀번호 필수, 신규 8자↑, 확인 일치
- [ ] 현재 비밀번호 불일치 시 에러 메시지
...
**완료 조건:**
- [ ] Desktop (≥768px): 사이드바 + 콘텐츠 레이아웃 확인
- [ ] Mobile (<768px): 전체 폭 콘텐츠 레이아웃 확인
- [ ] 터치 UX: 버튼 최소 크기 44px 확인
- [ ] 글로벌 스타일 및 CSS 변수 정의 완료
...
*본 문서는 TodoListApp의 전체 구현 작업을 Task 단위로 분해한 실행 계획서이다. 개발 진행에 따라 Task 완료 여부를 체크박스로 추적하고, 변경 사항이 있을 경우 변경 이력에 기록한다.*
